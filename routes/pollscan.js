...
router.get("/events", function(req, res, next) {
  if ("address" in req.query) {
    ...
    var myEvents = contractInstance.getPastEvents(
      ...
      function(err, logs) {
        ...
        if (logs) {
          getFirstAndLastBlockTimeStamp = async () => {
            let firstBlockDetails = await web3.eth.getBlock(
              logs[0]["blockNumber"]
            );
            let lastBlockDetails = await web3.eth.getBlock(
            ...
            if (firstBlockDetails.number === lastBlockDetails.number) {
              ...
              m =
                (lastBlockDetails.timestamp - firstBlockDetails.timestamp) /
                (firstBlockDetails.number - lastBlockDetails.number);
